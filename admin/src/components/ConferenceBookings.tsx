...
const ConferenceBookings: React.FC<BookingListProps> = ({ bookings }) => {
  ...
  useEffect(() => {
    ...
    // Only fetch conference details for bookings that don't include a snapshot
    conferenceIds.forEach(id => {
      const hasSnapshot = bookings.some(b => b.conference_id === id && (b.conference_name || b.conference_price));
      if (!hasSnapshot && !conferenceDetails[id]) {
        fetch(`${backendUrl}/api/conferences/${id}`)
          .then(res => res.json())
          .then(data => {
  ...
  const conference = booking.conference_name ? {
    id: booking.conference_id,
    name: booking.conference_name,
    price: booking.conference_price,
    size: booking.conference_size,
    max_users: booking.conference_max_users,
    amenities: booking.conference_amenities,
    image_urls: booking.conference_image_urls,
    description: booking.conference_description,
  } : conferenceDetails[booking.conference_id];
